...
func TestReceiveMessagePrefiltersStaleBeforeSignature(t *testing.T) {
	privateKey, err := ecdsa.GenerateKey(S256Curve, rand.Reader)
	assert.Nil(t, err)

	consensus := createConsensus(t, 10, 2, []*ecdsa.PublicKey{&privateKey.PublicKey})
	_, sp, _ := createRoundChangeMessageSigner(t, 11, 1, nil, privateKey)
