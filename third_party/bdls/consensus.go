...
const (
	...
	// reliableDecideProofReuseMinParticipants gates the per-height decide
	// certificate reuse optimization to clusters where repeated verification
	// of forwarded 2t+1 commit-proof bundles dominates. Seven-node clusters
	// keep the original simpler path, while 13+ node Fabric runs already show
	// enough forwarded proof traffic to benefit from reuse.
	reliableDecideProofReuseMinParticipants = 13
)
...
type Consensus struct {
	...
	// external signer callback; used when privateKey is nil
	signDigest func(digest []byte) (rBytes, sBytes []byte, err error)
	// public key when using external signer
	...
	// lazyStart keeps the instance passive until a proposal or inbound
	// consensus message activates the current height. This avoids empty
	// round-change churn for embedders whose block proposals arrive out of band.
	lazyStart bool
	active    bool
	...
	// pendingError holds the first error captured on the hot path after the
	// library-level panic-to-error conversion (see PR-A2). It is set by
	// setError, consumed by public entry points (ReceiveMessage, SubmitRequest,
	// Update, Propose) and then cleared. A single slot is sufficient because
	// the consensus state machine is strictly single-threaded.
	pendingError error

	// Reliable <decide> propagation (see PR-A3 / Section 9 of eprint 2019/1460).
	// When reliableDecide is true, the decide handler tracks the set of
	// distinct outer signers for each (height, round, stateHash) triple and
	// only advances to the next height once |signers| >= 2t+1. The node also
	// re-signs and re-broadcasts the decide exactly once per tuple so the
	// quorum can form across every honest replica.
	reliableDecide    bool
	decideAcks        map[decideAckKey]map[Identity]struct{}
	decideProofs      map[decideAckKey]*SignedProto
	decideRebroadcast map[decideAckKey]struct{}

	// Optional independent stage timeouts (see PR-A3 / Section 8.2). When a
	// field is zero the legacy latency-derived duration is used, preserving
	// pre-A3 behaviour for callers that do not set them.
...
// decideAckKey keys the reliable-decide tracking maps. The triple
// (height, round, stateHash) uniquely identifies a committed proposal.
type decideAckKey struct {
	height    uint64
	round     uint64
	stateHash StateHash
}

// setError records the first error encountered on the hot path. Later errors
// are dropped so the caller sees the original cause. Callers should return
// shortly after invoking this because the operation that raised the error has
// already been aborted in place of a panic.
func (c *Consensus) setError(err error) {
	if err == nil {
		return
	}
...
// consumePendingError returns any error recorded since the last call and
// clears it. Public entry points call this after running their body so the
// caller observes internal failures instead of seeing a silently-lost panic.
func (c *Consensus) consumePendingError() error {
	err := c.pendingError
	c.pendingError = nil
	return err
}
...
func (c *Consensus) init(config *Config) {
	...
	c.signDigest = config.SignDigest
	c.publicKey = config.PublicKey
	c.pubKeyToIdentity = config.PubKeyToIdentity
	c.enableCommitUnicast = config.EnableCommitUnicast
	c.lazyStart = config.LazyStart
	c.active = !config.LazyStart

	// PR-A3: reliable <decide> + independent Δ knobs. Zero values stay zero,
	// which reverts each duration to its latency-derived default.
	c.reliableDecide = config.ReliableDecide
	c.delta0 = config.Delta0
	c.delta1 = config.Delta1
	c.deltaPrime1 = config.DeltaPrime1
	c.delta2 = config.Delta2
	c.delta3 = config.Delta3
	...
	if c.privateKey != nil {
		...
		// signer path
		c.identity = c.pubKeyToIdentity(c.publicKey)
		c.curve = c.publicKey.Curve
	}
	...
	// and initiated the first <roundchange> proposal unless lazy start was
	// requested by the embedder.
	c.switchRound(0)
...
// scaleDelta applies the round-exponential backoff factor (1<<round) to a
// base duration and caps the result at MaxConsensusLatency. This is the
// shared scaling logic all five Δ knobs use.
func scaleDelta(base time.Duration, round uint64) time.Duration {
	d := base * (1 << round)
...
// roundchangeDuration returns Δ0 — the round-change stage timeout.
// When config.Delta0 is zero, derives 2*latency like pre-A3 code.
func (c *Consensus) roundchangeDuration(round uint64) time.Duration {
	base := c.delta0
	if base == 0 {
		base = 2 * c.latency
	}
	return scaleDelta(base, round)
}

// collectDuration is the leader's collection window for <roundchange>s.
// It shares Δ0's default (2*latency) but has its own knob via Δ'_1 in spirit;
// keep it tied to Delta0 for backwards compat when no override is given.
...
// lockDuration returns Δ1 — the lock stage timeout for non-leader. When
// config.Delta1 is zero, derives 4*latency as pre-A3 did.
func (c *Consensus) lockDuration(round uint64) time.Duration {
	base := c.delta1
	if base == 0 {
		// the leader-side override Δ'_1 applies only when the caller of
		// lockDuration is the leader. BDLS's current state machine uses
		// a single lockDuration call site, so we honour DeltaPrime1 when
		// the current identity IS the round leader and Δ1 is unset.
		if c.deltaPrime1 != 0 && c.roundLeader(round) == c.identity {
			base = c.deltaPrime1
		} else {
			base = 4 * c.latency
		}
...
// commitDuration returns Δ2 — the commit stage timeout. When config.Delta2
// is zero, derives 2*latency as pre-A3 did.
...
// lockReleaseDuration returns Δ3 — the lock-release stage timeout. When
// config.Delta3 is zero, derives 2*latency as pre-A3 did.
...
func (c *Consensus) verifyMessage(signed *SignedProto) (*Message, error) {
	return c.verifySignedMessage(signed, nil)
}

// verifySignedMessage verifies message signature against it's <r,s> & <x,y>,
...
func (c *Consensus) verifySignedMessage(signed *SignedProto, decoded *Message) (*Message, error) {
	if signed == nil {
...
// prefilterMessage rejects messages that are impossible to accept based only on
// local height/round/stage. It never accepts a message; anything still
// plausible continues through the normal signature and proof verification path.
func (c *Consensus) prefilterMessage(m *Message) error {
	if m == nil {
		return ErrMessageIsEmpty
	}

	switch m.Type {
	case MessageType_Nop, MessageType_Resync:
		return nil
	...
	case MessageType_LockRelease:
		if c.currentRound.Stage != stageLockRelease {
			return ErrLockReleaseStatus
		}
	case MessageType_Commit:
		if c.roundLeader(m.Round) != c.identity {
			return nil
		}
...
//
// The outer signer is validated strictly against the round leader unless
// reliable-decide mode (PR-A3) is enabled, in which case any known
// participant may re-sign and forward a <decide>. Security is unaffected:
// the 2t+1 inner <commit> proofs are the actual source of finality.
func (c *Consensus) verifyDecideMessage(m *Message, signed *SignedProto) error {
	...
	// Verify the outer signer.
	outer := c.pubKeyToIdentity(signed.PublicKey(c.curve))
	if c.reliableDecide {
		// Any known participant may forward a reliable decide. Reject
		// only unknown signers.
		known := false
		for _, id := range c.participants {
			if id == outer {
				known = true
		...
		// Strict: must be signed by the round leader.
		leaderKey := c.roundLeader(m.Round)
		if outer != leaderKey {
			return ErrDecideNotSignedByLeader
		}
...
func (c *Consensus) verifyDecideForward(m *Message) error {
	if m.State == nil {
		return ErrDecideEmptyState
	}
	if !c.stateValidate(m.State) {
...
// On marshal failure it records the error via setError and returns nil instead
// of panicking — the embedder observes the failure at the next public entry
// point (see consumePendingError).
func (c *Consensus) broadcast(m *Message) *SignedProto {
	...
	if c.privateKey != nil {
		...
		sp.SignWithSigner(m, c.signDigest, c.publicKey)
	}
	...
	if err != nil {
		c.setError(fmt.Errorf("%w: %v", ErrMarshalOutgoingMessage, err))
		return nil
	}
...
// On marshal failure it records the error via setError and returns early
...
func (c *Consensus) activateCurrentHeight(broadcast bool) {
	if !c.lazyStart {
		return
	}
	c.active = true
	if broadcast && c.currentRound != nil && c.currentRound.Stage == stageRoundChanging {
		c.broadcastRoundChange()
	}
}

func (c *Consensus) beginNextHeightRoundChange(now time.Time, extraDelay time.Duration) {
	if c.lazyStart {
		c.active = false
		c.rcTimeout = time.Time{}
		return
	}
	c.active = true
	c.rcTimeout = now.Add(c.roundchangeDuration(0) + extraDelay)
	c.broadcastRoundChange()
}
...
//
// If no direct error is returned but a prior hot-path operation recorded an
...
func (c *Consensus) ReceiveMessage(bts []byte, now time.Time) (err error) {
	...
	defer func() {
		...
		if err == nil {
			if pending := c.consumePendingError(); pending != nil {
				err = pending
			}
		}
	}()
...
func (c *Consensus) receiveMessage(bts []byte, now time.Time) error {
	...
	if err := c.prefilterMessage(m); err != nil {
		return err
	}

	// check message signature & qualifications
	m, err = c.verifySignedMessage(signed, m)
	if err != nil {
		return err
	}
	...
	if c.lazyStart && !c.active && m.Type != MessageType_Nop {
		c.activateCurrentHeight(false)
	}
		...
		if leaderKey == c.identity {
			...
			if c.currentRound.AddCommit(signed, m) {
				...
				if c.currentRound.NumCommitted() >= 2*c.t()+1 {
					...
					if c.reliableDecide {
						// PR-A3: in reliable mode the leader also waits
						// for 2t+1 distinct re-broadcasts before it moves
						// to the next height. Seed the ack tracker for
						// this (height,round,state) so the leader's own
						// loopback-delivered decide counts, and mark the
						// re-broadcast flag so the decide handler does
						// not re-sign what we just flooded.
						key := decideAckKey{
							height:    c.latestHeight + 1,
							round:     c.currentRound.RoundNumber,
							stateHash: c.stateHash(c.currentRound.LockedState),
						}
						if c.decideAcks == nil {
							c.decideAcks = make(map[decideAckKey]map[Identity]struct{})
							c.decideRebroadcast = make(map[decideAckKey]struct{})
						}
						if _, ok := c.decideAcks[key]; !ok {
							c.decideAcks[key] = make(map[Identity]struct{})
						}
						if c.numIdentities >= reliableDecideProofReuseMinParticipants {
							if c.decideProofs == nil {
								c.decideProofs = make(map[decideAckKey]*SignedProto)
							}
							c.decideProofs[key] = c.latestProof
						}
						c.decideRebroadcast[key] = struct{}{}
					} else {
						...
						c.beginNextHeightRoundChange(now, c.latency)
					}
				}
			}
		}
	...
	case MessageType_Decide:
		if c.reliableDecide {
			reuseProof := c.numIdentities >= reliableDecideProofReuseMinParticipants
			// Strongly-reliable <decide> path (PR-A3 / Section 9). Track
			// the set of distinct outer signers for this (height, round,
			// state) triple and only advance once the quorum threshold is
			// met. Until then, also re-sign and re-broadcast the decide
			// once so the quorum can form across every honest replica.
			key := decideAckKey{
				height:    m.Height,
				round:     m.Round,
				stateHash: c.stateHash(m.State),
			}
			if c.decideAcks == nil {
				c.decideAcks = make(map[decideAckKey]map[Identity]struct{})
				c.decideRebroadcast = make(map[decideAckKey]struct{})
			...
			if reuseProof {
				...
				if c.decideProofs[key] == nil {
					err := c.verifyDecideMessage(m, signed)
					if err != nil {
						return err
					}
					c.decideProofs[key] = signed
				} else {
					err := c.verifyDecideForward(m)
					if err != nil {
						return err
					}
				}
			...
			set, ok := c.decideAcks[key]
			if !ok {
				set = make(map[Identity]struct{})
				c.decideAcks[key] = set
			}
			outer := c.pubKeyToIdentity(signed.PublicKey(c.curve))
			set[outer] = struct{}{}

			// Remember the first valid proof we saw so CurrentProof() can
			// still return a witness for external verifiers (Fabric's
			// ValidateDecideMessage). The leader may have pre-populated
			// latestProof from broadcastDecide above; keep that when set.
			if c.latestProof == nil {
				if reuseProof {
					c.latestProof = c.decideProofs[key]
				} else {
					c.latestProof = signed
				}
			}

			// Re-sign + re-broadcast exactly once per decide to avoid a
			// gossip storm. Subsequent receipts just bump the signer set.
			if _, already := c.decideRebroadcast[key]; !already {
				c.decideRebroadcast[key] = struct{}{}
				c.broadcast(m)
			}

			// Hold the height until 2t+1 distinct signers have forwarded
			// this decide. This is what closes the Section-9 fork window:
			// even if the leader's flood misses 1..2t nodes, every honest
			// replica eventually observes the same 2t+1 attestations
			// before it lets go of the old height.
			if len(set) >= 2*c.t()+1 {
				if reuseProof {
					c.latestProof = c.decideProofs[key]
				} else {
				...
				c.beginNextHeightRoundChange(now, 0)
				// garbage-collect the tuple; keep the map for future heights
				delete(c.decideAcks, key)
				if c.decideProofs != nil {
					delete(c.decideProofs, key)
				}
				delete(c.decideRebroadcast, key)
			}
			return nil
		}
		...
		// Legacy (non-reliable) single-flood path — preserved so existing
		// embedders that do not set Config.ReliableDecide behave exactly
		// as before PR-A3.
		c.latestProof = signed
		...
		c.heightSync(m.Height, m.Round, m.State, now)
		// non-leader starts waiting for rcTimeout, unless lazy start keeps
		// the next height passive until a proposal arrives.
		c.beginNextHeightRoundChange(now, 0)
	...
	case MessageType_Resync:
		...
		for k := range m.Proof {
			...
			if err != nil {
				return fmt.Errorf("%w: %v", ErrMarshalResyncProof, err)
			}
			c.loopback = append(c.loopback, out)
		}
...
func (c *Consensus) Update(now time.Time) (err error) {
	...
	case stageRoundChanging:
		if c.rcTimeout.IsZero() {
			...
			// Formerly a panic. The assertion still indicates a programming
			// bug or corrupted internal state, but returning an error lets
	...
	case stageLockRelease:
		if c.lockReleaseTimeout.IsZero() {
			return ErrLockReleaseTimeoutNotSet
		}
...
// UpdateParticipants atomically swaps the consensus group to next. It is
// only safe to call between heights — when no <roundchange>, <lock>,
// <commit> or locks have been registered for the next height yet.
//
// This is the public, production-grade entry point for Fabric's
// channel-config reconfiguration flow (add/remove consenter), replacing
// the test-only AddParticipant helper. On success, the new participants
// become effective immediately and numIdentities is recomputed so the
// 2t+1 quorum math tracks the new group size.
//
// Returns ErrUpdateParticipantsHeightInFlight if any state for the in-
// progress height has been recorded, so the caller knows to retry after
// the current height finalises (or to drain it explicitly).
func (c *Consensus) UpdateParticipants(next []Identity) error {
	if len(next) < ConfigMinimumParticipants {
		return ErrConfigParticipants
	}
	// A height is considered in-flight if anything has been recorded for
	// the next height: roundchange / commit messages in the current round,
	// any round > 0 in the round list, or existing locks.
	if c.currentRound != nil {
		if c.currentRound.RoundNumber != 0 {
			return ErrUpdateParticipantsHeightInFlight
		}
		if c.currentRound.NumRoundChanges() > 0 {
			return ErrUpdateParticipantsHeightInFlight
		}
		if c.currentRound.NumCommitted() > 0 {
			return ErrUpdateParticipantsHeightInFlight
		}
	}
	if len(c.locks) > 0 {
	...
	// Swap the group and recompute cardinality.
	c.participants = append(c.participants[:0:0], next...)
	ids := make(map[Identity]bool, len(next))
	for _, id := range next {
		ids[id] = true
	}
	c.numIdentities = len(ids)
...
// SubmitRequest is an alias for ReceiveMessage that input to core incoming
// consensus messages, and returns error. Same pendingError semantics apply.
