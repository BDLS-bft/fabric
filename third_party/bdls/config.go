...
type Config struct {
	...
	// LazyStart suppresses the initial empty <roundchange> broadcast until the
	// instance has a local proposal or receives consensus traffic for the next
	// height. This keeps embedders with an external block-proposal path from
	// churning through empty rounds while idle.
	LazyStart bool
