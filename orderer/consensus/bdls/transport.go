...
func (p *peerAdapter) Send(msg []byte) error {
	// BDLS hands us freshly marshaled protobuf bytes and does not mutate them
	// after Send returns. Queue the slice directly to avoid copying large
	// proof-carrying lock/decide messages once per remote consenter.
	select {
	case p.sendC <- msg:
		return nil
	case <-p.stopC:
		return fmt.Errorf("bdls transport: peer adapter to consenter %d stopped", p.destination)
	}
}

func (p *peerAdapter) runSendLoop() {
	for {
		select {
		case msg := <-p.sendC:
			_ = p.sendDirect(msg)
...
func (p *peerAdapter) sendDirect(msg []byte) error {
	if p.logger != nil {
...
func (p *peerAdapter) SendSubmitAndWait(msg *orderer.SubmitRequest, timeout time.Duration, stopC <-chan struct{}) error {
	if p.logger != nil {
		p.logger.Debugf("bdls peerAdapter.SendSubmitAndWait: sending msg to %d", p.destination)
	}
	...
	report := func(err error) {
		if err != nil {
			submitErr.Store(err.Error())
		}
		once.Do(func() { close(sentC) })
	}

	if err := p.rpc.SendSubmit(p.destination, msg, report); err != nil {
		return err
	}
	...
	case <-sentC:
	case <-stopC:
		return fmt.Errorf("peer adapter to consenter %d stopped", p.destination)
	case <-timer.C:
		return fmt.Errorf("timed out after %s", timeout)
	}

	if err, ok := submitErr.Load().(string); ok {
