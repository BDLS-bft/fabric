...
func TestBuildBDLSConfig_PropagatesOptions(t *testing.T) {
	...
	require.True(t, cfg.EnableCommitUnicast, "Fabric should use BDLS's star-shaped commit path")
	require.True(t, cfg.LazyStart, "Fabric should not churn BDLS rounds before a block proposal exists")
...
func TestBuildBDLSConfig_NilOptionsOK(t *testing.T) {
	...
	require.True(t, cfg.EnableCommitUnicast)
	require.True(t, cfg.LazyStart)
}

func TestBuildBDLSConfig_DisableReliableDecide(t *testing.T) {
	consenters, _ := makeProtoConsenters(t, 4)
	md := &bdlsproto.ConfigMetadata{
		Consenters: consenters,
		Options: &bdlsproto.Options{
			DisableReliableDecide: true,
		},
	}
	cfg, err := buildBDLSConfig(md, 0)
	require.NoError(t, err)
	require.False(t, cfg.ReliableDecide)
	require.True(t, cfg.EnableCommitUnicast)
	require.True(t, cfg.LazyStart)
}

func TestBdlsCompactStateEnabled(t *testing.T) {
	require.False(t, bdlsCompactStateEnabled(nil))
	require.False(t, bdlsCompactStateEnabled(&bdlsproto.Options{}))
	require.True(t, bdlsCompactStateEnabled(&bdlsproto.Options{CompactBlockState: true}))

	t.Setenv("FABRIC_BDLS_COMPACT_STATE", "false")
	require.False(t, bdlsCompactStateEnabled(&bdlsproto.Options{CompactBlockState: true}))

	t.Setenv("FABRIC_BDLS_COMPACT_STATE", "true")
	require.True(t, bdlsCompactStateEnabled(&bdlsproto.Options{}))
}
