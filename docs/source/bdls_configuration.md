...
* `CompactBlockState` (`compact_block_state`)
...
`CompactBlockState` is intended for high-throughput channels. When enabled,
BDLS signs and proves a compact block reference instead of embedding the full
marshalled block in every proof message; the full block is still disseminated
over the orderer cluster channel before commit.
